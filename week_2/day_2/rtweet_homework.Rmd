...
```{r}
tweets <- tweets %>% 
  mutate(unquoted = if_else(is_quote == FALSE, favorite_count, 0))
sum(tweets$unquoted)
```
...
tweets <- tweets %>% 
  mutate(number_of_characters = nchar(text))
mean(tweets$number_of_characters)
```
They do not match, in fact there is a significant difference.
Wonder if spaces count for nchar() and are not included in 'display_text_width'
...
```
```{r}
tweets_n_info <- left_join(tweets, info, by = "tweet_id")
```
...
```{r}
codeclan_hashtags <- tweets_n_info %>% 
