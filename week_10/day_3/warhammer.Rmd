...
houses_trim <- houses %>% 
  mutate(renovated = if_else(yr_renovated == 0, FALSE, TRUE),
         waterfront = as.logical(houses$waterfront)) %>% 
...
```{r}
houses_trimr <- houses_trim %>% 
  select(-sqft_basement)
```

```{r}
alias(price ~ ., houses_trimr)
```
...
Systematically build a regression model containing up to **four** main effects (remember, a main effect is just a single predictor with coefficient), testing the regression diagnostics as you go
    * splitting datasets into numeric and non-numeric columns might help `ggpairs()` run in manageable time, although you will need to add either a `price` or `resid` column to the non-numeric dataframe in order to see its correlations with the non-numeric predictors.
    ...
    never managed to finish as I had a hectic day, but I would have a gander at the correlations, and 
    boxplots to see what relationships look interesting, pick a first few vars to assess against price as
    a model then have a check of the assumptions before looking at any model params. if it satisfies the 
    assumptions and I'm happy with the accuracy of the model I will pick the best option, move on and
    repeat the process with the remaining vars of interest. Using anova we can see how much of a
    difference these new vars are adding to the model. keep what is making a sig difference or is of
    particular interest. A last step could be to test interactions where we feel it may be interesting
    (splitting a ggpairs by colour on a var of interest may help determine what is interesting).
